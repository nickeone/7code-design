...
const CS_PHOTO_COVERS: Record<string, { src: string; alt: string }> = {
...
function CSCover({ kind, label, slug }: { kind: string; label?: string; slug?: string }) {
  const photo = slug ? CS_PHOTO_COVERS[slug] : undefined;
  if (photo) {
    return (
  ...
  const map: Record<string, { cls: string; icon: React.ReactElement }> = {
...
function CaseStudies({ limit }: { limit?: number }) {
  const items = [
          ...
          {items.map((item, i) => {
            const inner = (
              <img
                src={item.src}
                alt={item.alt}
                className={"trust-logo" + ((item as any).className ? " " + (item as any).className : "")}
                loading="lazy"
              />
            );
            return (item as any).href ? (
              <a key={i} href={(item as any).href} target="_blank" rel="noopener noreferrer"
...
// Expertise section
function Expertise({ headTitle = "Our Expertise", headDesc = "We specialize in key sectors, delivering tailored digital solutions that drive growth, efficiency, and innovation." }: { headTitle?: string; headDesc?: string }) {
  const items = [
    {
...
function Services() {
  const services = [
    ...
    { icon: Icon.cpu, slug: "llm-agent-development", title: "LLM & Agent Development", desc: "Custom LLM integrations and multi-step agent pipelines built for production, eval-gated every release, not just demos." },
...
export { TrustedBy, CSCover, CaseStudies, WhyChoose, VerifiedAccredited, Expertise, Services, Testimonials, CTAStrip, CS_PHOTO_COVERS };
