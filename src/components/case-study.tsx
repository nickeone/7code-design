...
const CASES: Record<string, any> = {
  ...
  "drum-bun": {
    ...
    tagline: "InsurTech meets automotive compliance: verify, purchase, and track every Romanian car obligation, RCA insurance, ITP technical inspection, road vignette, from one React Native app with an OCR document pipeline.",
...
// ──────────────────────────────────────────────────────────────────
function PhoneMockup({ accent = "var(--cyan-500)" }: { accent?: string }) {
  if (CSD.slug === "daily8") {
    return (
...
function PhotoHero({ src, alt, urlBar, caption }: { src: string; alt: string; urlBar?: string; caption?: string }) {
  return (
...
function BrowserMockup({ kind = "calendar" }: { kind?: string }) {
...
function FactsList({ inline = false }: { inline?: boolean }) {
  const facts = [
...
function ResultsStrip({ dark = false }: { dark?: boolean }) {
  return (
    <div className={"csd-results" + (dark ? " csd-results--dark" : "")}>
      {CSD.results.map((r: any, i: number) => (
...
function TestimonialCard({ flat = false }: { flat?: boolean }) {
  ...
  return (
    <div className={"csd-quote" + (flat ? " csd-quote--flat" : "")}>
      ...
      <div className="csd-quote-foot">
        <div className="csd-quote-avatar">{CSD.testimonial.name.split(" ").map((s: string) => s[0]).join("")}</div>
        <div>
        ...
        <article className="csd-long-article">
          ...
          <section id="csd-challenge">
            ...
            <ul className="csd-bullets">
              {CSD.challenge.pains.map((p: string, i: number) => (
          ...
          <section id="csd-approach">
            ...
            <div className="csd-pillars">
              {CSD.approach.pillars.map((p: any, i: number) => (
          ...
          <div className="csd-metric-hero-grid">
            <div className="csd-metric-hero-text">
              ...
              <div className="csd-metric-hero-stats">
                {CSD.results.slice(0, 3).map((r: any, i: number) => (
        ...
        <div className="container">
          <div className="csd-visual-split">
            <div className="csd-visual-split-text">
              ...
              <p>{CSD.challenge.body}</p>
              <ul className="csd-bullets">
                {CSD.challenge.pains.map((p: string, i: number) => <li key={i}><span className="csd-bullet-dot"/>{p}</li>)}
              </ul>
            </div>
            {hasPhoneShot() && (
        ...
        <aside className="csd-sticky-aside">
          <div className="csd-sticky-card">
            ...
            <div className="csd-sticky-results">
              {CSD.results.slice(0, 2).map((r: any, i: number) => (
...
// ─────────────────────────────────────────────────────────────────
function CaseStudyPage({ slug = "wholesum", variant = "longread" }: { slug?: string; variant?: string }) {
