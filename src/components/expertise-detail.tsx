...
import { CTAStrip, CaseStudies } from './sections';
...
// Shared icon helper
// ─────────────────────────────────────────────────────────────────
function ExpertiseIcon({ name, style }: { name: string; style?: React.CSSProperties }) {
...
// ─────────────────────────────────────────────────────────────────
function ExpertiseCoverBg({ kind }: { kind: string }) {
...
// ─────────────────────────────────────────────────────────────────
function ExpertiseDetailPage({ slug = "healthcare" }: { slug?: string }) {
  const exp = EXPERTISE[slug] || EXPERTISE["healthcare"];
  const nextSlug = exp.next.slug;
  const relatedCases = exp.cases.map((s: string) => ({
    slug: s,
    title: s,
    kind: exp.kind,
    meta: [exp.title],
        ...
        <div className="container exp-hero-inner">
          <div className="exp-hero-text">
            ...
            <div className="exp-hero-stats">
              {exp.stats.map((s: { v: string; l: string }, i: number) => (
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="exp-cap-grid">
            {exp.capabilities.map((c: { icon: React.ComponentType<any>; title: string; desc: string }, i: number) => {
              const I = c.icon;
              return (
      ...
      <section className="section section--alt">
        <div className="container">
          ...
          <div className="exp-process">
            {exp.process.map((p: { step: string; title: string; desc: string }, i: number) => (
        ...
        <section className="section">
          <div className="container svc-faq-wrap">
            ...
            <div className="svc-faq">
              {exp.faqs.map((f: { q: string; a: string }, i: number) => (
