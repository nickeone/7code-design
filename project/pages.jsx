...
function HomePage() {
  ...
  if (!home) {
    ...
    return <div className="page"><TrustedBy /><CaseStudies limit={6} /><Services /><CTAStrip /></div>;
  }

  const { spec, blocks } = home;
  return (
    <div className="page">
      <HeroBlock hero={blocks.hero} cta={spec.cta} />
      <TrustStripBlock trustStrip={blocks.trustStrip} />
      <RouterBlock router={blocks.router} />
      <RichSection nodes={blocks.diagnosis.rich} />
      <ProofBlock proof={spec.proof} />
      <CtaBlock cta={spec.cta} />
      <RichSection nodes={blocks.whyUs.rich} alt />
      <RichSection nodes={blocks.packages.rich} />
      <FaqBlock faq={blocks.faq} />
      <Testimonials />
      <CloseBlock close={blocks.close} cta={spec.cta} />
    </div>
  );
}
...
function AboutPage() {
  ...
  const stats = [
    { v: foundedYear, l: "Founded in Cluj-Napoca" },
  ...
  const team = [
    { name: "Nicu Mardari", role: "CEO & Founder", initial: "N", photo: "/project/uploads/authors/nicu-mardari.jpg", bio: `Nicu Mardari founded 7code in Cluj-Napoca in ${foundedYear}, building it from a boutique software studio into a specialist AI product engineering firm serving clients across the UK, EU, UAE, and US. As CEO, Nicu leads business development, client strategy, and commercial operations, working directly with founders, CTOs, and technology leaders on AI automation and product engineering engagements. He brings a decade of experience in nearshore software delivery and AI-first product strategy.` },
    { name: "Igor Mardari", role: "CTO & Co-Founder", initial: "I", photo: null, bio: `Igor Mardari is the co-founder and CTO of 7code, responsible for technical strategy, engineering standards, and the architecture of client AI systems. Since co-founding 7code in ${foundedYear}, Igor has led the technical evolution of the company from general software development into AI-native product engineering and cloud agentic infrastructure. He brings deep expertise in LLM system design, cloud architecture at scale, and AI product evaluation frameworks, and sets the technical quality standards that underpin 7code's senior-only delivery model.` },
...
function FAQPage() {
  const faqs = [
    ...
    { q: "What is 7code’s Clutch rating?", a: "7code holds a 4.8-star rating on Clutch.co based on 11 verified client reviews (as of 2025). Clutch reviews cover engagement quality, delivery timeliness, communication, and willingness to recommend. All reviews are verified by Clutch’s research team through direct client interviews. View 7code’s Clutch profile at clutch.co/profile/7code." },
    ...
    { q: "What does 7code do?", a: "7code is an AI-native nearshore software engineering partner based in Cluj-Napoca, Romania. We build custom software, LLM integrations, agentic workflows, and system integrations for startups and SMEs in the UK, EU, and UAE. Our team is senior-only (70%+ senior engineers) and we operate an AI-first delivery model — every project is assessed for automation and AI opportunities from day one." },
    ...
    { q: "What are 7code's engineering rates?", a: "7code's rates: Mid-level engineers €30–35/hour; Senior engineers €40–50/hour; Architect-level engineers €60–80/hour. These are 40–60% below London market rates with equivalent or higher seniority. All rates are transparent with no hidden markups." },
    ...
    { q: "How does 7code ensure quality?", a: "Quality is enforced through: senior-only hiring (70% of engineers are senior-level); multi-stage technical screening including case studies; delivery governance with weekly sprint demos and monthly steering committees; and AI-specific quality gates — golden datasets, regression tests on every model version change, and human-in-the-loop checkpoints for high-stakes outputs. We track velocity, predictability, bug escape rate, and team utilisation as standard KPIs." },
