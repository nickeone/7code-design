...
const POSTS = [
  ...
  // ── Additional articles transferred from medium.com/7code ──────
  {
    slug: "react-node-modern-digital-products",
    cat: "Engineering",
    title: "The modern way to build digital products using React and Node",
    subtitle: "React on the front end, Node on the back end, and a single language across the stack. The combination has carried us from small apps to enterprise platforms, and it still wins on hiring, maintenance, and speed.",
    date: "Sep 04, 2023",
    read: "5 min read",
    author: { name: "Cindy-Cristina Daraban", role: "Business Development Manager", initial: "C", photo: "project/uploads/authors/cindy-daraban.jpg" },
    cover: "ink",
    illus: "circuit",
    body: [
      { type: "lead", text: "Modern digital products demand both responsive interfaces and the ability to handle large numbers of concurrent users without falling over. React on the front end and Node on the back end are the combination that has carried us through projects ranging from small applications to enterprise-scale platforms, with one shared language across the stack and a component model that scales as the team grows." },
      { type: "h2", text: "Why React for the front end" },
      { type: "p", text: "React's component-based approach lets us break complex user interfaces into smaller, reusable pieces, each maintained in isolation and composed into screens that stay coherent as the product evolves. The unidirectional data flow makes the path from state to UI predictable, which means fewer regressions when teams move fast and more confidence when changes need to land before a release." },
      { type: "h2", text: "Why Node for the back end" },
      { type: "bullets", items: [
        "JavaScript on both client and server, eliminating context switches between languages and enabling shared code across the stack",
        "Event-driven, non-blocking I/O that handles many concurrent connections efficiently, ideal for real-time products",
        "A massive ecosystem of well-maintained packages that accelerates everything from authentication to job queues",
        "Excellent fit with serverless and container deployments, keeping infrastructure cost in line with traffic",
      ]},
      { type: "callout", text: "Same language, same patterns, same review process across front end and back end. The team multiplies, the stack stays simple." },
      { type: "h2", text: "How we use them together" },
      { type: "p", text: "We pair React with Next.js when SEO and rendering performance matter, and with a thinner SPA stack when the product is dashboard-heavy. On the server we lean on Nest.js for structured backends and plain Node for lightweight APIs and serverless workers. The shared TypeScript types between the two layers eliminate a class of integration bugs that traditionally cost teams days during release." },
      { type: "h2", text: "Why this matters for our clients" },
      { type: "p", text: "React and Node aren't a fashion choice, they're a hiring choice and a maintenance choice. The talent pool is deep, the documentation is exhaustive, and the patterns are familiar to any engineer your team will hire after we hand the project over. That's the point: we build with technologies our clients can keep growing without us." },
    ],
  },
  {
    slug: "aws-beanstalk-to-ecs-fargate-perspective",
    cat: "Engineering",
    title: "Why I migrated from AWS Beanstalk to ECS + Fargate: a personal perspective",
    subtitle: "After six months of running production workloads on Elastic Beanstalk, I moved everything to ECS on Fargate. The decision wasn't ideological, it was operational. Here's how Beanstalk and ECS compare in practice.",
    date: "Oct 25, 2023",
    read: "6 min read",
    author: { name: "Igor Mardari", role: "CTO & Co-founder", initial: "I", photo: "project/uploads/authors/igor-mardari.jpg" },
    cover: "defence",
    illus: "gears",
    body: [
      { type: "lead", text: "After six months of building production services on AWS Elastic Beanstalk, I moved everything to ECS on Fargate, and stopped looking back. The decision wasn't ideological, it was operational. Beanstalk solves the first 30% of a deployment story very well, then makes the next 70% harder than it should be. ECS plus Fargate kept the operational simplicity I wanted and gave me the control I needed." },
      { type: "h2", text: "Where Beanstalk shines" },
      { type: "p", text: "Beanstalk is the right answer when the team is small, the application is simple, and you want a working deployment in an afternoon. It abstracts away the load balancer, the auto-scaling group, and the health checks, lets you push code, and gets out of your way. For a first deployment, that's exactly what you want." },
      { type: "h2", text: "Where Beanstalk hurts" },
      { type: "bullets", items: [
        "Configuration drift, the .ebextensions and platform-hooks layer becomes opaque the moment you need anything non-default",
        "Slow deploys, immutable updates that take 6 to 10 minutes per environment, multiplied by the number of environments",
        "Rolling back a bad deployment is awkward, the cleanest path is often to redeploy the previous artefact",
        "Multi-container apps live in a parallel universe, ECS managed by Beanstalk, and you inherit the worst of both abstractions",
      ]},
      { type: "h2", text: "Why ECS + Fargate" },
      { type: "p", text: "ECS is the middle ground I had been looking for: containers as the unit of deployment, full control over the task definition, and clean integration with the rest of the AWS ecosystem. Fargate removes the EC2 capacity planning, so I'm not babysitting an autoscaling group, and the cost per task is predictable enough to budget against." },
      { type: "callout", text: "Beanstalk gets you to production. ECS keeps you there. The point at which you outgrow Beanstalk is the point at which the migration costs less than the next surprise." },
      { type: "h2", text: "What I'd do differently" },
      { type: "p", text: "Start with ECS and Fargate from day one if there's any chance you'll need multi-service deployments, blue-green rollouts, or fine-grained IAM per service. The learning curve is real, but it's a one-time investment, and it pays back the first time you deploy a new microservice without filing a Beanstalk environment ticket." },
    ],
  },
  {
    slug: "aws-beanstalk-to-ecs-simple-migration",
    cat: "Engineering",
    title: "Migrate from AWS Beanstalk to AWS ECS in a simple way",
    subtitle: "Once you decide to move from Beanstalk to ECS, the next question is how to get there without a multi-week project. A small bash script can stand up a working ECS service end-to-end: VPC, security group, cluster, task, service, and logs.",
    date: "Oct 25, 2023",
    read: "5 min read",
    author: { name: "Igor Mardari", role: "CTO & Co-founder", initial: "I", photo: "project/uploads/authors/igor-mardari.jpg" },
    cover: "ink",
    illus: "gears",
    body: [
      { type: "lead", text: "Once the decision to move from Beanstalk to ECS is made, the next question is how to get there without a multi-week project. This is the script-and-config approach we used to deploy the simplest possible ECS service on AWS, with VPC, security group, ECS cluster, task, service, and CloudWatch logs, all reproducible from a single bash run." },
      { type: "h2", text: "What 'simple' means here" },
      { type: "p", text: "The goal is a minimum-viable ECS deployment: a container running on Fargate, behind a security group, logging to CloudWatch, and reachable from outside the VPC. No load balancer, no service discovery, no autoscaling. Once the simple version is running, every additional component is a small, isolated change rather than a green-field rewrite." },
      { type: "h2", text: "What the script provisions" },
      { type: "bullets", items: [
        "A VPC with public subnets, sized to match the number of availability zones you want to deploy into",
        "A security group with the minimum ingress rules required to reach the application port",
        "An ECS cluster on Fargate, with the IAM execution and task roles wired up correctly",
        "A task definition pointing at a container image, with CloudWatch log configuration baked in",
        "A service that pins the desired count at one task, ready to scale once the deployment is verified",
        "A CloudWatch log group with a sensible retention policy, so logs don't accumulate forever",
      ]},
      { type: "callout", text: "The fastest way to demystify ECS is to provision it once from scratch. After that, every Beanstalk-style abstraction makes sense, because you can map it back to the underlying primitives." },
      { type: "h2", text: "What the script deliberately leaves out" },
      { type: "p", text: "An application load balancer, blue-green deployments, and CodeDeploy integration are the next sensible additions, but adding them in the same script makes the script unreadable. We prefer to layer them on as separate, reviewable changes once the simple service is live." },
      { type: "h2", text: "Why this matters for teams considering ECS" },
      { type: "p", text: "If your team's mental model of ECS is fuzzy, the simple migration removes the mystery. Once a developer has run the script, broken something, fixed it, and deployed a real container, ECS feels like infrastructure you can reason about, not a black box. That confidence is what unblocks the move from Beanstalk to a deployment story you can actually grow into." },
    ],
  },
  {
    slug: "tech-audit-legacy-project",
    cat: "Engineering",
    title: "Why a tech audit is important for a legacy project",
    subtitle: "A legacy codebase doesn't tell you the truth on its own. A tech audit is the structured exercise that turns years of decisions into a real picture of where the system is, what's at risk, and where the next investment should go.",
    date: "Oct 11, 2023",
    read: "6 min read",
    author: { name: "Cindy-Cristina Daraban", role: "Business Development Manager", initial: "C", photo: "project/uploads/authors/cindy-daraban.jpg" },
    cover: "finance",
    illus: "blueprint",
    body: [
      { type: "lead", text: "A legacy codebase doesn't tell you the truth on its own. It tells you a version of the truth filtered through years of decisions, half-remembered tradeoffs, and missing context. A tech audit is the structured exercise that turns that fog into a snapshot, a real picture of what the system does, where the risks are, and what the next investment should be. Without it, every roadmap is a guess." },
      { type: "h2", text: "What a tech audit actually is" },
      { type: "p", text: "A tech audit is a comprehensive examination of a system, application, or IT setup designed to assess its capacity to meet the organisation's needs, surface threats, identify inefficiencies, and highlight areas for improvement. The output is a report, but the value is the alignment, every stakeholder ends up with the same understanding of where the system is." },
      { type: "h2", text: "What a tech audit covers" },
      { type: "bullets", items: [
        "Architecture review: how the components fit together, where the coupling is, and which boundaries are load-bearing",
        "Security posture: known vulnerabilities, dependency hygiene, secret management, and data handling",
        "Performance and scalability: where the bottlenecks are now and where they will appear under projected load",
        "Code quality: test coverage, technical debt hotspots, and the parts of the codebase that fear refactoring",
        "Operational readiness: monitoring, alerting, runbooks, and the realities of on-call",
        "Compliance and governance: what the system has to satisfy, and whether it actually does",
      ]},
      { type: "h2", text: "Why legacy projects need it most" },
      { type: "p", text: "Greenfield projects are easy to reason about, the original team is still there, the decisions are documented, and the stack is current. Legacy projects are the opposite, and the gap between what the team thinks the system does and what it actually does grows quietly. A tech audit closes that gap, often by surfacing things the new team didn't know to ask about." },
      { type: "callout", text: "An audit isn't an indictment, it's a map. The legacy project that gets one is the one that survives the next migration, the next regulator, the next traffic spike." },
      { type: "h2", text: "What changes after the audit" },
      { type: "p", text: "Investment decisions stop being theological and start being evidence-based. Refactoring proposals get scoped against measured pain points, not assumed ones. Security work gets prioritised against documented risk, not vendor noise. The audit converts opinion into data, and data is what unlocks budget." },
    ],
  },
  {
    slug: "enoki-cloud-desktop-collaboration",
    cat: "Case Study",
    title: "Enoki: empowering collaboration with the first developer-native cloud desktop",
    subtitle: "How 7Code partnered with Enoki to build a cloud desktop designed from the ground up for engineering teams, with deep IDE integration, real-time collaboration, and a code-review workflow that feels native rather than bolted on.",
    date: "Jul 04, 2023",
    read: "5 min read",
    author: { name: "Cindy-Cristina Daraban", role: "Business Development Manager", initial: "C", photo: "project/uploads/authors/cindy-daraban.jpg" },
    cover: "cyan",
    illus: "skyline",
    body: [
      { type: "lead", text: "Enoki came to us with a sharp idea: a cloud desktop designed from the ground up for engineering teams, not retrofitted. The product had to handle real-time collaboration, deep IDE integration, and a code-review workflow that felt native rather than bolted on. We partnered with the Enoki team to build it, with a tech stack tuned for the latency and concurrency this kind of product demands." },
      { type: "h2", text: "The product brief" },
      { type: "p", text: "Enoki's bet is that engineering teams collaborate badly because the tools force them to. Sharing a screen is a workaround. Pair-programming over a video call is a workaround. A real cloud desktop, where the workspace itself is the shared artefact, removes the workarounds. Our job was to make that workspace fast, reliable, and pleasant to live in for hours at a stretch." },
      { type: "h2", text: "What we built" },
      { type: "bullets", items: [
        "A Next.js front end optimised for low-latency rendering across a globally distributed user base",
        "A Nest.js backend organised around clean service boundaries, so collaboration features could ship independently",
        "AWS infrastructure tuned for the network-heavy workload, with edge caching and regional fan-out",
        "Serverless Python microservices for the analytics, indexing, and review pipelines",
        "A code-review surface that integrates with the desktop session itself, removing the context switch into a separate review tool",
      ]},
      { type: "h2", text: "What changed for the users" },
      { type: "p", text: "Reported numbers from the Enoki team after launch: code-review time reduced by up to 50%, and team collaboration effectiveness up by around 30%. Those are big claims, but they reflect a small, repeated win, the developer no longer has to switch tools to comment on a teammate's work, and the teammate no longer has to context-switch to respond." },
      { type: "callout", text: "The product wins when the workflow disappears. Cloud desktops aren't about virtualising hardware, they're about making collaboration the default state of the team, not the special-case action." },
      { type: "h2", text: "Why this engagement worked" },
      { type: "p", text: "Enoki had a clear product vision and a high standard for engineering quality. Our role was to bring the AWS, Next.js, and Nest.js experience that turned the vision into infrastructure they could grow on, with a code-review workflow that the founding team itself wanted to use every day. The product is in production, the engineering team is shipping, and the partnership keeps producing." },
    ],
  },
  {
    slug: "7code-team-passionate-young",
    cat: "Operations",
    title: "Not just young and restless: passionate and striving for success",
    subtitle: "The strongest argument for a small, motivated team is that the team itself is the product. Why we hire narrow, invest in people, and build the culture around keeping outstanding engineers engaged for the long run.",
    date: "Dec 13, 2022",
    read: "4 min read",
    author: { name: "Alexandra Claudia", role: "People & Culture", initial: "A", photo: "project/uploads/authors/alexandra-claudia.jpg" },
    cover: "energy",
    illus: "launch",
    body: [
      { type: "lead", text: "The strongest argument for a small, motivated team is that the team itself is the product. Every line of code, every commit, every late-night fix carries the fingerprints of the people who wrote it. We don't compete on headcount, we compete on the quality of the people who choose to spend their working hours with us, and on a culture that gives them reasons to stay." },
      { type: "h2", text: "People before process" },
      { type: "p", text: "Process scales a team that has the right people in it. With the wrong people, process is a ceiling, not a floor. Our hiring bar is intentionally narrow, we look for engineers and product thinkers who care about craft, who push back when something is wrong, and who take ownership without being asked. The rest of the company is built around keeping that kind of person engaged." },
      { type: "h2", text: "What that looks like in practice" },
      { type: "bullets", items: [
        "Compensation packages that genuinely reflect the responsibility we ask people to take",
        "Performance bonuses tied to real outcomes, not management theatre",
        "Team-building activities that the team actually wants to attend",
        "Friday evenings dedicated to recreation and socialising, not pretend culture",
        "Direct access to senior leadership, no hierarchy of reasons why a question can't get answered",
      ]},
      { type: "callout", text: "Talent is not retained by job titles. It's retained by the daily experience of being trusted to do good work." },
      { type: "h2", text: "Why young teams move fast" },
      { type: "p", text: "Younger teams have an unfair advantage when the technology landscape is shifting, fewer assumptions to unlearn, more hunger to ship the next thing, and a comfort with experimentation that older organisations sometimes lose. Combined with senior technical leadership, that energy turns into delivery velocity that bigger competitors struggle to match." },
      { type: "h2", text: "What we ask for in return" },
      { type: "p", text: "Honesty when something is broken. Curiosity when a problem is unfamiliar. Discipline when the deadline is real. We're a small company, and the cost of a quiet under-performer is higher than in a large organisation. The flip side, the cost of an outstanding contribution is also more visible. People who do excellent work here see their work recognised." },
    ],
  },
  {
    slug: "new-office-cluj-napoca",
    cat: "Operations",
    title: "Say hello to our new office",
    subtitle: "After months of looking, we found the office we wanted: great light, room to grow, nature around the building, and a view over Cluj-Napoca that makes Monday mornings genuinely pleasant. The new address is Cetății 39.",
    date: "Sep 07, 2022",
    read: "3 min read",
    author: { name: "7Code Team", role: "Team Update", initial: "7", photo: "project/uploads/authors/7code-team.jpg" },
    cover: "health",
    illus: "skyline",
    body: [
      { type: "lead", text: "After months of looking, we found the office we wanted: a space with great light, room to grow, nature around the building, and a view over Cluj-Napoca that makes coming in on a Monday morning genuinely pleasant. We moved in over a long weekend and were back at full productivity by the end of the week. The new address is Cetății 39, and we'd love to have visitors." },
      { type: "h2", text: "Why the office still matters" },
      { type: "p", text: "We work hybrid, and a lot of our engineering happens remotely. But there's a quality of work that only happens when the team is in the same room: the whiteboard sessions that resolve a tricky architecture decision in twenty minutes instead of three Slack threads, the spontaneous code reviews that uncover a subtle bug, the lunches that turn into product ideas. The new space is built for those moments." },
      { type: "h2", text: "What we picked it for" },
      { type: "bullets", items: [
        "Location, easy to reach from anywhere in Cluj-Napoca, and close to good food and coffee",
        "Light, large windows that turn the workday into something humans actually want to do",
        "Acoustics, dedicated rooms for focus work and dedicated rooms for the meetings that need to be loud",
        "Room to grow, more desks than we currently need, so we can hire without renegotiating space",
        "A great view, which is a small thing until you spend eight hours a day looking out the window",
      ]},
      { type: "h2", text: "How we moved in" },
      { type: "p", text: "We picked up the keys on a Friday and were running by Monday. Three days, a few rounds of furniture assembly, and a celebration party that the whole team enjoyed. The feedback from the team was unanimous: the new office is what we needed." },
      { type: "callout", text: "Offices don't make great teams, but the right office makes a great team's work easier. Ours has done exactly that." },
      { type: "h2", text: "Drop by" },
      { type: "p", text: "If you're in Cluj-Napoca and want to talk shop, drop us a line. The kettle is on, the coffee is good, and the view is, as advertised, very nice." },
    ],
  },
];
