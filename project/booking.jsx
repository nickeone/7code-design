...
const { useEffect: useEffectBK } = React;
...
function BookPage({ slug }) {
  ...
  const route = bookingRoutes[slug] || bookingRoutes.discovery || { calLink: "7code/discovery-call" };
    ...
    <div className="page">
      <section className="page-hero">
        <div className="container">
          <span className="eyebrow" style={{ justifyContent: "center" }}>{page.eyebrow}</span>
          <h1>{page.title}</h1>
          <p>{page.intro}</p>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container" style={{ maxWidth: 720, margin: "0 auto" }}>
          <CalEmbed calLink={route.calLink} />

          <p style={{ marginTop: 20, textAlign: "center", color: "var(--slate-500)", fontSize: 14 }}>
            {page.reassurance}
          </p>

          <div style={{ marginTop: 24, textAlign: "center", paddingTop: 24, borderTop: "1px solid var(--slate-200)" }}>
            <p style={{ marginBottom: 12, color: "var(--slate-500)", fontSize: 14 }}>
              Would rather not book a slot yet?
            </p>
            <a href="/contact" className="btn btn--ghost">
              Use the contact form instead <Icon.arrow />
            </a>
          </div>
        </div>
      </section>
